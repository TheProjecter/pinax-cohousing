...
class EventRelationTester(models.Model):
    """
    >>> tester = EventRelationTester(name="Meeting 1")
    >>> tester.save()
    >>> start = datetime.datetime.now()
    >>> end = start + datetime.timedelta(minutes=30)
    >>> event = Event(start=start, end=end, title=tester.name)
    >>> event.save()
    >>> rel = EventRelation.objects.create_relation(event, tester)
    >>> rel
    <EventRelation: Meeting 1(None)-EventRelationTester object>
    """
